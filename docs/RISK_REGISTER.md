...
|---|---|---|---|---|---|---|---|
| R01 | Target leakage in lag, rolling, imputation, or segmentation logic | Medium | Critical | Validation score is implausibly high; performance collapses on later folds | M2 freezes lag-one minimum target features, fold-local transforms, reporting-only segments, lagged future prices, recursive Seasonal Naive behavior, and cutoff tests | Remove affected features, invalidate runs, and rerun all comparisons | Monitoring |
...
| R09 | Intermittent-demand metrics become unstable or misleading | Medium | High | Percentage errors are undefined or dominated by near-zero denominators | M3 implements weighted/scaled primary metrics, reports null/count for zero denominators, retains per-series distributions, and treats WAPE as secondary | Remove unsuitable metric from selection policy while retaining it as descriptive | Monitoring |
...
| R12 | Final test is inspected repeatedly | Medium | Critical | Hyperparameters or features change after final-test review | Protocol `m2-v1` records the final window as locked; validation reads no target columns; require a frozen champion configuration before final-target access | Declare contamination, create a new untouched terminal window if possible | Monitoring |
| R13 | Reproducibility fails across machines | Medium | High | Clean environment cannot repeat core results | M3 run IDs exclude local paths and two same-environment runs produced identical core checksums; dependencies and fixtures remain pinned | Complete a clean-machine/container reproduction before release; publish any platform limitation | Monitoring |
